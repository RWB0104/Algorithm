...
public class Main
{
	...
	public static void main(String[] args) throws IOException
	{
		...
		for (int caseNum = 0; caseNum < t; caseNum++)
		{
			...
			//System.out.println(dp[both][n / 2]);
			...
			if (enemy[top][1] + enemy[top][n / 2] <= w)
			{
				...
				//System.out.println(dp[bottom][n / 2]);
			}
			
			//아래만 연결된 경우
			else if (enemy[bottom][1] + enemy[bottom][n / 2] <= w)
			{
				dp = new int[3][n / 2 + 1];
				dp[bottom][1] = 1;
				dp[top][1] = INF;
				...
				//System.out.println(dp[top][n / 2]);
			}
			
			//둘 다 연결된 경우
			else if (enemy[top][1] + enemy[top][n / 2] <= w && enemy[bottom][1] + enemy[bottom][n / 2] <= w)
			{
				dp = new int[3][n / 2 + 1];
				...
				//System.out.println(dp[both][n / 2 - 1]);
			}
